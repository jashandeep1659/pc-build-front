...
const PrimaryStorageChoice = (props) => {
    ...
    useEffect(() => {
        api.get(`${baseURL}primarystorages/`)
            .then((response) => {
    ...
    const OptionRender = proccesorsList.map((item, index) => {
        try {
            if (item.slug == selectedBuild.primary_Storage.slug) {
                return null;
            }
        } catch (e) {}
        return (
    ...
    const valueUpdater = (value) => {
        const updatedValues = { primarystorage: value };
                    ...
                    <form>
                        ...
                        >
                            ...
                            selectedBuild.primary_Storage.name ? (
                                <option
                                    value={selectedBuild.primary_Storage.slug}
                                >
                                    {selectedBuild.primary_Storage.name}
                                </option>
                            ) : (
...
export default PrimaryStorageChoice;
