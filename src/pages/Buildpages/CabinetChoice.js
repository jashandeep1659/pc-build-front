...
const CabinetChoice = (props) => {
    ...
    const OptionRender = proccesorsList.map((item, index) => {
        try {
            if (item.slug == selectedBuild.cabinet.slug) {
                return null;
            }
        } catch (e) {}
        return (
    ...
    const valueUpdater = (value) => {
        const updatedValues = { cabinet: value };
    ...
    return (
        <div>
            <div className="text-center">
                ...
                <h1 className="font-bold text-4xl px-3 text-center">Cabinet</h1>
                <div className="my-8">
                    <form>
                        <select
                            name="processors"
                        ...
                        >
                            ...
                            selectedBuild.cabinet.name ? (
                                <option value={selectedBuild.cabinet.slug}>
                                    {selectedBuild.cabinet.name}
                                </option>
                            ) : (
